...
class ResumeTextExtractor:
    ...
    def _extract_from_image(self) -> str:
        """Extract text from image file with cloud OCR fallback."""
        try:
            ...
            if is_render:
                ...
                try:
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        print("❌ Tesseract not found via 'which' command")
                except Exception as e:
                    print(f"❌ Error running 'which tesseract': {str(e)}")
            else:
                # Local development
                try:
                    pytesseract.get_tesseract_version()
            ...
            if tesseract_available:
                try:
                    ...
                    if extracted_text and extracted_text.strip():
                        ...
                        return "Warning: No readable text found in image using local OCR."
                except Exception as e:
                    print(f"❌ Local OCR failed: {str(e)}")
                    # Fall through to cloud OCR
            
            # Fallback: Use a simple image-to-text approach or return helpful message
            print("🔄 Falling back to alternative text extraction...")
            
            # For now, return a helpful message
            return """
            Error: OCR (Optical Character Recognition) is temporarily unavailable on this server.

            To process image resumes:
            1. Convert your image to PDF format using online tools
            2. Or use text-based formats like .docx or .txt
            3. Or try again later as we're working to fix the OCR service

            Supported formats that work: PDF, DOCX, TXT
                    """.strip()
            
        except Exception as e:
            return f"Error processing image: {str(e)}. Please try converting to PDF format."
